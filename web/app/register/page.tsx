    ...
    <div className="min-h-screen relative flex flex-col lg:flex-row overflow-x-hidden selection:bg-blue-100">
      ...
      <div className="relative z-10 flex-1 flex flex-col h-screen overflow-y-auto">
        <div className="flex-1 flex items-center justify-center p-4 lg:p-8">
          <div className="backdrop-blur-xl bg-white/70 p-6 md:p-8 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.12)] max-w-2xl w-full border border-white/40 animate-in fade-in slide-in-from-right-4 duration-500">
            
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-blue-600 font-black text-[10px] uppercase tracking-[0.2em] mb-0.5">Onboarding Process</p>
                <h2 className="text-2xl font-black text-slate-900 tracking-tight">Step {step} of 5</h2>
              </div>
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((s) => (
                  <div key={s} className={`w-8 h-1 rounded-full transition-all duration-500 ${step >= s ? 'bg-blue-600' : 'bg-slate-200'}`} />
                ))}
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className={`space-y-3 animate-in fade-in slide-in-from-right-4 duration-500 ${step === 1 ? 'block' : 'hidden'}`}>
                {/* WhatsApp Option at the very top (immediately after the title) */}
                <div className="grid grid-cols-1 gap-2.5">
                  <a 
                    href={`https://wa.me/${process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || 'your-number'}?text=Hi! I want to register my Mahalla`}
                    className="flex items-center justify-between p-3.5 rounded-2xl bg-emerald-50 border border-emerald-100 hover:bg-emerald-100/50 transition-all group"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-[10px] font-black text-emerald-900 uppercase tracking-widest">Register via WhatsApp</div>
                        <div className="text-[9px] font-bold text-emerald-600/70">Fastest, no forms required</div>
                      </div>
                    </div>
                  ...
                  <div className="relative py-1">
                    ...
                    <div className="relative flex justify-center text-[9px] font-black uppercase tracking-widest"><span className="bg-white px-3 text-slate-400">Or use web form</span></div>
                  </div>
                </div>

                <div className="space-y-1 mb-2">
                  <h2 className="text-lg font-black text-slate-900 tracking-tight">Let's Get Started</h2>
                  <p className="text-[11px] text-slate-500 font-bold leading-relaxed">
                    Provide your contact information below. This person will be the primary administrator.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 gap-3.5">
                  <div className="space-y-1">
                    ...
                    <input required={step === 1} name="fullName" type="text" className="w-full px-4 py-2.5 bg-white/50 border border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-300 focus:bg-white focus:border-blue-600 focus:ring-4 focus:ring-blue-50 outline-none transition-all text-sm" placeholder="Mohamed" />
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                    <div className="space-y-1">
                      <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Email Address</label>
                      <input required={step === 1} name="email" type="email" className="w-full px-4 py-2.5 bg-white/50 border border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-300 focus:bg-white focus:border-blue-600 focus:ring-4 focus:ring-blue-50 outline-none transition-all text-sm" placeholder="mohamed@example.com" />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Phone Number</label>
                      <input required={step === 1} name="phone" type="tel" pattern="0[0-9]{9}" maxLength={10} title="Phone number must start with 0 and be exactly 10 digits long (e.g. 0771234567)" className="w-full px-4 py-2.5 bg-white/50 border border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-300 focus:bg-white focus:border-blue-600 focus:ring-4 focus:ring-blue-50 outline-none transition-all text-sm" placeholder="077XXXXXXX" />
                    </div>
                  </div>
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Proposed Mahalla Name</label>
                    <input required={step === 1} name="mahallaName" type="text" className="w-full px-4 py-2.5 bg-white/50 border border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-300 focus:bg-white focus:border-blue-600 focus:ring-4 focus:ring-blue-50 outline-none transition-all text-sm" placeholder="e.g. Grand Mosque Association" />
                  </div>
                  ...
                  <div className="p-3 bg-slate-50/50 rounded-2xl border border-slate-100">
                    <label className="flex items-start gap-3 cursor-pointer group">
                      <input 
                        ...
                        className="mt-0.5 w-5 h-5 rounded border-2 border-slate-300 text-blue-600 focus:ring-blue-500 transition-all cursor-pointer" 
                      />
                      <span className="text-xs font-bold text-slate-600 leading-relaxed group-hover:text-slate-900 transition-colors">
                        {instructionLang === 'en' ? (
                          <>
              ...
              <div className="flex items-center gap-3 pt-3">
                {step > 1 && (
                  <button type="button" onClick={() => setStep(step - 1)} className="flex-1 py-3.5 px-6 rounded-2xl font-black text-slate-600 bg-white border border-slate-200 hover:bg-slate-50 transition-all active:scale-95 uppercase tracking-widest text-xs">
                    Back
                  </button>
                )}
                <button 
                  ...
                  type="submit" 
                  className={`flex-[2] flex items-center justify-center gap-2.5 py-3.5 px-6 rounded-2xl font-black text-white bg-slate-900 hover:bg-blue-600 transition-all shadow-lg active:scale-95 uppercase tracking-widest text-xs disabled:opacity-50 disabled:cursor-not-allowed`}
                >
