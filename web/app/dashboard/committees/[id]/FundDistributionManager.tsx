...
import {
  ...
  Eye, Trash2, Edit3, User, Building2, Paperclip
} from "lucide-react";
import {
...
export function FundDistributionManager({ committeeId, terms }: { committeeId: string; terms: any[] }) {
  ...
  const filtered = requests.filter((r) =>
    viewFilter === "active"
      ? !["DISBURSED", "REJECTED"].includes(r.status)
      : ["DISBURSED", "REJECTED"].includes(r.status)
  );
      ...
      <div className="p-6 border-b border-slate-100 flex flex-wrap justify-between items-center gap-4 bg-slate-50/50">
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${viewFilter === "completed" ? "bg-slate-900 text-white shadow-lg" : "bg-white text-slate-500 border border-slate-200"}`}
          >
